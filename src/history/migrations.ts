...
// 資料格式的升級轉換：migrations[N] 把版本 N 的資料轉成版本 N + 1。
...
export const upgradeHistory = (fromVersion: number, history: StoredHistory): StoredHistory => {
  let upgraded = history
  for (let version = fromVersion; version < SNAPSHOT_SCHEMA_VERSION; version++) {
