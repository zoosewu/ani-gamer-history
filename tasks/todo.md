...
- 設計由比較頁選定：動畫瘋用「書籤」、anime1 採用推薦方案，單集文章的書籤改放在標題列最右側
- 共用 `src/pages/marker.css`：琥珀色書籤（避開動畫瘋的青、粉與 anime1 的綠），支援動畫瘋深色模式
- 動畫瘋分集按鈕：書籤掛在格子右上；滑過顯示「本機紀錄・第 N 集・看到 mm:ss / mm:ss」
- anime1 系列頁／文章頁：書籤在標題列最右側、日期旁「上次看到…」；文章頁打開其他集時顯示「上次看到第 N 集 →」
- anime1 首頁列表：以 `?cat=系列 id` 對應，書籤掛在該列上緣並接「看到第 N 集」；DataTables 換頁重畫時自動重新標記
- 驗證：單元測試 77 → 84；端對端 21 個情境；真實巴哈與 anime1 CSS 下檢查位置（標題左緣與其他集一致、書籤貼齊右側且與標題頂端對齊）、同格並存、深色模式、列表換頁
