...
## 追加：本機歷史紀錄卡片標題用完整寬度（2026-09-17）
- 巴哈的繼續觀看卡片把標題寬度扣掉 72px（hover 時 86px）留給觀看數；本機歷史紀錄沒有觀看數，在 `.ani-gamer-history` 範圍內覆寫為完整寬度（hover 時左右各留 8px）
- 驗證（真實巴哈 CSS）：標題寬度 145px → 217px（= 卡片內容寬度）；hover 131px → 201px，左右間距 8px / 8px

## 修正：舊版同步造成 anime1 紀錄重複（2026-09-17）
...
## 端對端測試移進 repo + 「功能」頁開關 + 同步時間移到齒輪右邊（2026-09-18）
- [x] 1. 端對端測試搬進 `e2e/`（含 anime1 fixture），`npm run test:e2e`，CI 執行並在失敗時上傳截圖
- [x] 2. 偏好設定新增 `hiddenMarkers`，兩個欄位各自容錯
- [x] 3. 動畫瘋分集書籤、anime1 標記依開關顯示
- [x] 4. 設定 dialog 的「顯示」頁改為固定出現的「功能」頁
- [x] 5. 同步時間移到齒輪右邊
- [x] 6. 驗證：lint / test / build / e2e、乾淨環境跑 e2e、真實巴哈 CSS 截圖
- 端對端測試：`e2e/run.mjs` + `e2e/fixtures/anime1/`，`npm run test:e2e`；CI 在 build 後安裝 Chromium 執行，失敗時上傳 `e2e/out/` 截圖
- 偏好設定新增 `hiddenMarkers`（依來源，之後的網站自動適用），`hiddenSources` / `hiddenMarkers` 各自容錯；不影響紀錄資料格式，不需調版號
- 「顯示」頁改為固定出現的「功能」頁：「上次觀看標記」（動畫瘋固定出現，其他網站有紀錄才出現）與「首頁清單」（有其他網站紀錄才出現）；共用 `ToggleRow`
- 同步時間移到齒輪右邊
- 驗證：單元測試 128 → 133；端對端 23 → 25 個情境（功能頁的初始狀態、關閉標記後兩站標記立即消失且仍記錄進度、重新整理後保留、不觸發推送、同步時間位置）；真實巴哈 CSS 下標題、齒輪、同步時間垂直中心都在同一條線上

## 資料版本改用 semver、本機只存一份最新版資料（2026-09-18）
- [x] `DATA_VERSION = '2.0.0'`（`src/history/dataVersion.ts`）：MAJOR.MINOR 比腳本新就拒絕，只有 PATCH 較新時接受；`schemaVersion` 改為給 0.9.0 以前看的閘門（2.0.x 寫 2，之後寫 3）
- [x] 本機只用一個 `history` 欄位，格式和雲端相同；`animeHistory`、`animeHistory.v2` 遷移後刪除，舊分頁再寫出來也會在下次載入時收進來並刪除
- [x] 本機資料比腳本新時鎖定（`storage.lockedBy`）：不讀寫本機、不同步、不能匯入匯出；已開著的分頁收到較新資料時也會鎖定；首頁、同步指示、設定頁提示更新
- [x] 相容性測試依 MAJOR.MINOR 分組；凍結測試資料帶自己的版本；打包快取換資料夾
- [x] 文件改寫為「資料版本（dataVersion）」
- 驗證：
  - 單元測試 133 → 141；相容性測試 15 個（0.4.0～0.7.1 為 1.0.0、0.8.0 為 2.0.0 並通過無損來回）
  - 端對端 25 → 27 個情境：遷移後只剩 `history`（含 `dataVersion`）、雲端資料 2.1.0 時提示更新且不寫回、本機資料 2.1.0 時首頁鎖定且不發任何請求、開著的分頁收到 3.0.0 後停止寫入
  - 用 GitHub 上真實的 0.8.0 發佈檔驗證：更新前開著的 0.8.0 分頁只寫 `animeHistory.v2`、不碰 `history`；新版下次載入把它收進來並刪除，最後只剩 `history`
